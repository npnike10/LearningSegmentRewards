...
def objective(trial):
    ...
    checkpoint_path = f"./checkpoint/hparam_tuning/rationales/qwen3-4B-rm-rat1-optuna-trial-{trial_id}"
...
def extract_validation_loss(checkpoint_path):
    ...
    if "eval/eval_loss" in ea.Tags()["scalars"]:
...
if __name__ == "__main__":
    # Create study
    study = optuna.create_study(direction="minimize", study_name="rm_tuning")

    print("Starting RM hyperparameter optimization...")
    ...
    if study.trials:
        ...
        with open("best_rm_params.json", "w") as f:
            json.dump(study.best_params, f, indent=2)

        print("\nBest parameters saved to best_rm_params.json")
    else:
