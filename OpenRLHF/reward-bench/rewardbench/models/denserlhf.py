import torch
from openrlhf.models import get_llm_for_sequence_regression_denserm
import torch.nn.functional as F
from typing import Dict


def build_denserlhf_rm(model_name, **kwargs):
    reward_model = get_llm_for_sequence_regression_denserm(model_name, "reward", **kwargs)
...
class DenserSegRMFPipeline:
    ...
    def tokenize_batch_element(
        ...
        if len(prompt_tokens["input_ids"]) + len(response_tokens["input_ids"]) > self.max_length:
            response_tokens = {
                k: v[: self.max_length - len(prompt_tokens["input_ids"])] for k, v in response_tokens.items()
            }
    ...
    def __call__(self, samples_prompt, samples_response, entropy_thresholds, **kwargs):
        ...
        tokenized_batch = [
            self.tokenize_batch_element(prompt, response) for prompt, response in zip(samples_prompt, samples_response)
        ]

        response_ids, response_masks, response_labels = zip(
            *[
                (batch["response_input_ids"], batch["response_attention_mask"], batch["response_labels"])
        ...
        response_ids = torch.stack(
            [self.pad_left(seq, max_length, self.tokenizer.pad_token_id) for seq in response_ids]
        ).to(torch.cuda.current_device())
        response_masks = torch.stack([self.pad_left(seq, max_length, 0) for seq in response_masks]).to(
            torch.cuda.current_device()
        )
        response_labels = torch.stack(
            [self.pad_left(seq, max_length, self.label_pad_token_id) for seq in response_labels]
        ).to(torch.cuda.current_device())
        ...
        with torch.no_grad():
            all_values_sequences, _ = self.model(
                response_ids, attention_mask=response_masks, return_output=True, return_every_step_reward=True
            )
