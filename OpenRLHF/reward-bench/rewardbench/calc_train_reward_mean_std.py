...
class RewardDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        chosen_text = self.tokenizer.apply_chat_template(
            [chosen_response], tokenize=False, add_generation_prompt=False
        )
        rejected_text = self.tokenizer.apply_chat_template(
            [rejected_response], tokenize=False, add_generation_prompt=False
        )
...
def collate_fn(batch):
    ...
    return {"prompt": prompts, "chosen_text_only": chosen_responses, "rejected_text_only": rejected_responses}
...
def main():
    ...
    dataset = datasets.load_dataset(
        "parquet", data_files="../data_processing/preferences/rationales_preference_train.parquet", split="train"
    )
    ...
    for step, batch in enumerate(dataloader):
        ...
        if not is_dpo:
            ...
            (
                ...
                chosen_original_rewards,
                chosen_segment_position_rewards,
                chosen_additional_info,
            ) = reward_pipe(batch["prompt"], batch["chosen_text_only"], entropy_thresholds, **reward_pipeline_kwargs)
            (
                rewards_rejected,
                rejected_segment_num,
                rejected_sequence_length,
                rejected_segment_length,
                rejected_original_rewards,
                rejected_segment_position_rewards,
                rejected_additional_info,
            ) = reward_pipe(batch["prompt"], batch["rejected_text_only"], entropy_thresholds, **reward_pipeline_kwargs)
            ...
            for chosen_scores, rejected_scores in zip(score_chosen_batch, score_rejected_batch):
                chosen_best, rejected_best = max(zip(chosen_scores, rejected_scores), key=lambda x: x[0] - x[1])
    ...
    if accelerator.is_main_process:
        ...
        stats["final_segment_rewards"] = {
            "mean": final_results["final_segment_rewards_mean"],
            "std": final_results["final_segment_rewards_std"],
        }
        ...
        with open(
            os.path.join(
                args.output_dir,
                f"{model_type}_{dataset_type}_{args.entropy_threshold}_{args.agg_func}_{args.segment_method}_segment_position_rewards_samples{len(dataset)}.json",
            ),
            "w",
        ) as f:
            json.dump(
        ...
        chosen_filename = os.path.join(
            args.output_dir,
            f"{model_type}_{dataset_type}_{args.entropy_threshold}_{args.agg_func}_{args.segment_method}_chosen_segment_text_rewards_samples{len(dataset)}.json",
        )
        save_data_to_json(chosen_all_additional_info, chosen_filename)
        rejected_filename = os.path.join(
            args.output_dir,
            f"{model_type}_{dataset_type}_{args.entropy_threshold}_{args.agg_func}_{args.segment_method}_rejected_segment_text_rewards_samples{len(dataset)}.json",
        )
        save_data_to_json(rejected_all_additional_info, rejected_filename)
        stats_filename = os.path.join(
            args.output_dir,
            f"{model_type}_{dataset_type}_{args.entropy_threshold}_{args.agg_func}_{args.segment_method}_evaluation_statistics_samples{len(dataset)}.json",
        )
