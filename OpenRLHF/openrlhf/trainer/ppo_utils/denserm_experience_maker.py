...
from openrlhf.models import Actor_DenseRM
from openrlhf.models.utils import compute_reward_denserm, masked_mean
...
class NaiveExperienceMaker(ABC):
    ...
    def get_generations(self, prompts, **generate_kwargs):
        ...
        sequences, attention_mask, action_mask, output_seqs, eos_token_present = self.actor.generate(
    ...
    def get_adjusted_mean_std(self, position_percentage):
        ...
        def convert_keys_to_float(obj):
            if isinstance(obj, dict):
                return {
                    float(k) if k.replace(".", "", 1).isdigit() else k: convert_keys_to_float(v)
        ...
        if not hasattr(self, "_cached_params"):
            param_path = self.strategy.args.norm_params_path
            with open(param_path, "r") as f:
                self._cached_params = convert_keys_to_float(json.load(f))

        p = (
            self._cached_params.get(self.model_type, {})
    ...
    def get_segment_rewards(
        self, sequences, attention_mask, action_mask, eos_token_present, device, base_action_entropy=None
    ):
        with torch.no_grad():
            ...
            if self.segment_method == "peak":
                segment_note = "peak"
                all_segments_end = self.segment_data_peak(
                    rm_sequence, action_mask, entropy_thresholds, base_action_entropy
                )

            all_values_sequences, _ = self.reward_model(
                rm_sequence, attention_mask=rm_attn_mask, return_output=True, return_every_step_reward=True
            )
            ...
            for i in range(batch_size):
                ...
                last_eos_index = action_mask[i].nonzero().max().item() if action_mask[i].nonzero().size(0) > 0 else 0
                ...
                for pos, end_pos in enumerate(segment_end_positions):
                    ...
                    if self.reward_model.normalize_reward:
                        if self.ppo_reward_type in [
                            ...
                            if self.verbose:
                                print(
                                    f"self.ppo_reward_type: {self.ppo_reward_type}, "
                ...
                if self.ppo_reward_type in ["every_segment_last", "same_segment_last", "segment_normalization"]:
                    ...
                    if hasattr(self, "response_penalty_length") and response_length > self.response_penalty_length:
                    ...
                    if eos_token_present:
                        rewards[i] = self.log_sum_exp_temperature * torch.logsumexp(
                            torch.tensor(segment_values) / self.log_sum_exp_temperature, dim=0
                        )
                    else:
                        rewards[i] = self.penalty
            ...
            if self.strategy.args.print_index % 4 == 0:
                for sample_idx in range(2):
                    reward_length_pairs = list(
                        zip(sample_segment_rewards[sample_idx], sample_segment_lengths[sample_idx])
                    )
                    self.strategy.print(
    ...
    def make_experience(self, prompts: Union[str, List[str]], **generate_kwargs):
        ...
        sequences, attention_mask, action_mask, output_seqs, eos_token_present = self.get_generations(
        ...
        r, avg_segment_lengths = self.get_segment_rewards(
            ...
            eos_token_present,
            device="cuda",
            base_action_entropy=base_action_entropy,
        )

        reward, kl = compute_reward_denserm(
            r,
            self.kl_ctl.value,
            action_log_probs,
        ...
        advantage_list = []
        action_mask_list = []
        advantage_list.append(advantage.flatten())
        action_mask_list.append(action_mask.flatten())

        advantages_vector = torch.cat(advantage_list, dim=0).float()
        action_masks_vector = torch.cat(action_mask_list, dim=0)
