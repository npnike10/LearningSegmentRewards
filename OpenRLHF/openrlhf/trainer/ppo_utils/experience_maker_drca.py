...
from openrlhf.models.utils import compute_approx_kl, compute_reward, compute_reward_drca, masked_mean
...
from openrlhf.utils.reward_hooks import create_reward_hook, apply_reward_hook
...
class RemoteExperienceMaker(ABC):
    def __init__(
        ...
        **kwargs,
        ...
        self.reward_hook = create_reward_hook(
            args=self.strategy.args, tokenizer=self.tokenizer, reward_model_group=self.reward_model_group
        )
    ...
    @torch.no_grad()
    def make_experience_batch(self, rollout_samples, drca_beta, unit_extraction) -> List[Experience]:
        """
        ...
        experiences, no_units = self.make_experience(samples_list, drca_beta, unit_extraction)

        # Process experiences (reward shaping, etc.)
        experiences = self.compute_advantages_and_returns(experiences, no_units)
        return experiences

    @torch.no_grad()
    def make_experience(
        self, samples_list: List[Experience], drca_beta: float, unit_extraction: str
    ) -> List[Experience]:
        """
        ...
        args = self.strategy.args
        device = "cpu"
        need_entropy = unit_extraction == "entropy"
        ...
        action_log_probs_ref = self.actor_model_group.async_run_method_batch(
            ...
            attention_mask=attention_mask_list,
            return_output=[need_entropy] * len(sequences_list),
            return_entropy=[need_entropy] * len(sequences_list),
        )
        ...
        if need_entropy:
            results = ray.get(action_log_probs_ref)[::duplicate_factor]
            action_log_probs_list = []
            entropy_list = []
            for result_batch in results:
                for log_probs, entropy in result_batch:
                    action_log_probs_list.append(log_probs)
        ...
        if need_entropy:
            for i, samples in enumerate(samples_list):
                samples.entropy = entropy_list[i]

        samples_list, no_units = apply_reward_hook(
            self.reward_hook, samples_list, drca_beta, unit_extraction, args.colocate_all_models, duplicate_factor
        )
    ...
    def compute_advantages_and_returns(
        self, experiences: List[Experience], no_units: bool, **kwargs
