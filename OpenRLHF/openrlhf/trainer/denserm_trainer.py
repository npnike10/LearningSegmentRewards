...
class RewardModelTrainer(ABC):
        ...
        for epoch in range(start_epoch, self.epochs):
            ...
            for (
                ...
                (
                    ...
                    aux_loss,
                    chosen_segment_rewards,
                    rejected_segment_rewards,
                ...
                if margin is not None:
                    margin = torch.tensor(margin, dtype=torch.float32).to(
                        torch.cuda.current_device()
                    )  # Ensure margin is a float tensor
                    margin = (
                        margin.unsqueeze(1).repeat(1, self.num_thresholds).view(-1)
                    )  # Repeat margin for each threshold

                if self.args.entropy_regularizer:
                    # compute segment credit entropy regularization term
                    # compute segment credit entropy regularization term
                    # For each batch element separately (since variable segment counts)
                    chosen_entropies = []
                    rejected_entropies = []

                    for seg_rewards in chosen_segment_rewards:
                        # seg_rewards shape: [num_segments]
                        probs = F.softmax(seg_rewards, dim=0)
                        log_probs = F.log_softmax(seg_rewards, dim=0)
                        entropy = -torch.sum(probs * log_probs)  # scalar
                        # Normalize by segment count (your "T_segment length")
                        normalized_entropy = entropy / len(seg_rewards)
                        chosen_entropies.append(normalized_entropy)

                    for seg_rewards in rejected_segment_rewards:
                        probs = F.softmax(seg_rewards, dim=0)
                        log_probs = F.log_softmax(seg_rewards, dim=0)
                        entropy = -torch.sum(probs * log_probs)
                        normalized_entropy = entropy / len(seg_rewards)
                        rejected_entropies.append(normalized_entropy)

                    # Average across batch
                    chosen_entropy_mean = torch.stack(chosen_entropies).mean()
                    rejected_entropy_mean = torch.stack(rejected_entropies).mean()
                    combined_entropy = chosen_entropy_mean + rejected_entropy_mean

                    # regularized loss
                    loss = self.loss_fn(chosen_reward, reject_reward, margin) - self.args.lambda_reg * combined_entropy
                else:
        ...
        with torch.no_grad():
            ...
            for (
                ...
                (
                    chosen_reward,
                    reject_reward,
                    batch_chosen_segment_num,
                    batch_reject_segment_num,
                    batch_chosen_response_length,
                    batch_reject_response_length,
                    _,
                    chosen_segment_rewards,
                    rejected_segment_rewards,
                ) = self.concatenated_forward(
    ...
    def concatenated_forward(self, model, chosen_ids, c_mask, chosen_labels, reject_ids, r_mask, rejected_labels):
        input_ids, att_masks, input_labels = self.concatenated_inputs(
            chosen_ids, c_mask, chosen_labels, reject_ids, r_mask, rejected_labels
        )
        ...
        all_values_sequences, output = model(
            input_ids, attention_mask=att_masks, return_output=True, return_every_step_reward=True
        )
        ...
        for i in range(batch_size):
            for thresh in range(self.num_thresholds):
                ...
                if len(segment_end_positions) > 1:
                    first_segment_length = segment_end_positions[0] - (
                        (input_labels[i] != -100).nonzero(as_tuple=True)[0][0].item() - 1
                    )
                    subsequent_segment_lengths = torch.tensor(segment_end_positions[1:]) - torch.tensor(
                        segment_end_positions[:-1]
                    )
                    segment_lengths = torch.cat(
                        (torch.tensor([first_segment_length]), subsequent_segment_lengths.float())
                    )
                else:
                ...
                if i < batch_size // 2:
                    ...
                    chosen_segment_rewards.append(torch.stack(batch_segments_values))
                else:
                    rejected_avg_segment_counts.append(len(segment_end_positions))
                    rejected_segment_rewards.append(torch.stack(batch_segments_values))
        ...
        chosen_segment_lengths = (
            all_segments_lengths[: self.num_thresholds * batch_size // 2].view(-1, self.num_thresholds).mean(dim=1)
        )
        ...
        chosen_avg_segment_counts = (
            torch.tensor(chosen_avg_segment_counts).float().view(-1, self.num_thresholds).mean(dim=1)
        )
        rejected_avg_segment_counts = (
            torch.tensor(rejected_avg_segment_counts).float().view(-1, self.num_thresholds).mean(dim=1)
        )
