...
from openrlhf.trainer.ppo_utils.experience_maker_drca import RemoteExperienceMaker
            ...
            # TensorBoard
                ...
                for k, v in logs_dict.items():
                    if k == "generated_samples":
                        ...
                        text, reward, outcome_reward = v
                        if reward is not None:
                            reward_str = ", ".join([f"{r:.4f}" for r in reward])
                        formatted_text = f"Sample:\n{text}\n\nToken-level Rewards: {reward_str}\n\nOutcome Reward: {outcome_reward:.4f}"
...
@ray.remote
class DRCATrainer(BasePPOTrainer):
    """
        ...
        for episode in range(episode, args.num_episodes):
            ...
            for _, rand_prompts, labels in self.prompts_dataloader:
                ...
                experiences = self.experience_maker.make_experience_batch(
                    rollout_samples, self.args.drca_beta, self.args.unit_extraction
                )
                ...
                # outcome_reward = experiences[0].info.get("outcome_reward", experiences[0].rewards)[0]
                status["generated_samples"] = [
                    sample0,
                    experiences[0].info["reward"][0],
                    experiences[0].info["outcome_reward"][0],
                ]
