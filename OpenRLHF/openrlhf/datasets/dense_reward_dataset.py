...
class PreferenceDataset(Dataset):
    def __init__(
        ...
        for data in tqdm(dataset, disable=not self.strategy.is_rank_0()):
            ...
            if (
                "chosen-rating" in data
                and "rejected-rating" in data
                and isinstance(data["chosen-rating"], (int, float))
                and isinstance(data["rejected-rating"], (int, float))
            ):
                margin = data["chosen-rating"] - data["rejected-rating"]
            elif (
                "chosen_score" in data
                and "rejected_score" in data
                and isinstance(data["chosen_score"], (int, float))
                and isinstance(data["rejected_score"], (int, float))
            ):
    ...
    def tokenize_batch_element(
        ...
        elif self.model_type == "qwen3-4B-sft-rat2":
        ...
        if len(prompt_tokens["input_ids"]) + longer_response_length > self.max_length:
            chosen_tokens = {
                k: v[: self.max_length - len(prompt_tokens["input_ids"])] for k, v in chosen_tokens.items()
            }
            rejected_tokens = {
    ...
    def __getitem__(self, idx):
        prompt_text, chosen_text, reject_text = (
            self.prompt_text[idx],
            self.complete_chosen_text[idx],
            self.complete_reject_text[idx],
        )
    ...
    def collate_fn(self, item_list):
        ...
        max_length = max(max(len(seq) for seq in chosen_ids), max(len(seq) for seq in reject_ids))
