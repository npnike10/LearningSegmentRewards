...
def compute_reward_denserm(
    ...
    kl = compute_approx_kl_denserm(log_probs, log_probs_base, action_mask=action_mask)
    kl_reward = -kl_coef * kl

    r = r.clamp(min=-10, max=10)

    if ppo_reward_type == "segment_last_logsumexp" or ppo_reward_type == "segment_last_avg":
        eos_indices = action_mask.size(1) - 1 - action_mask.long().fliplr().argmax(dim=1, keepdim=True)
        last_reward = torch.zeros_like(kl).scatter_(dim=1, index=eos_indices, src=r.unsqueeze(1).to(kl.dtype))
        reward = last_reward + kl_reward
    else:
...
def log_probs_from_logits_denserm(logits: torch.Tensor, labels: torch.Tensor) -> torch.Tensor:
