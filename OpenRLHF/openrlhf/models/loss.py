...
class ValueLoss(nn.Module):
    ...
    def forward(
        ...
        # # Normalize returns and values together for stable training
        # mask_flat = action_mask.flatten()
        # returns_flat = returns.flatten()
        # num_valid = mask_flat.sum()
        # return_mean = (returns_flat * mask_flat).sum() / num_valid
        # return_var = ((returns_flat - return_mean).pow(2) * mask_flat).sum() / num_valid
        # return_rstd = return_var.clamp(min=1e-8).rsqrt()
        # returns = (returns - return_mean) * return_rstd
        # values = (values - return_mean) * return_rstd
        # old_values = (old_values - return_mean) * return_rstd
