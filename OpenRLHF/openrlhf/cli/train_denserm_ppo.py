...
from openrlhf.datasets import CustomPromptDataset, SFTDataset
from openrlhf.datasets.utils import blending_datasets_denserm
from openrlhf.models import Actor_DenseRM, get_llm_for_sequence_regression_denserm
from openrlhf.trainer.denserm_ppo_trainer import PPOTrainer
from openrlhf.utils import get_strategy_denserm, get_tokenizer


def train(args):
    # configure strategy
    strategy = get_strategy_denserm(args)
    ...
    if args.init_critic_from_rm:
        ...
        critic = get_llm_for_sequence_regression_denserm(
    ...
    if not args.remote_rm_url:
        reward_model = get_llm_for_sequence_regression_denserm(
            args.reward_pretrain,
            "reward",
            normalize_reward=args.normalize_reward,
            use_flash_attention_2=args.flash_attn,
    ...
    # prepare datasets
    prompts_data = blending_datasets_denserm(
        args.prompt_data,
    ...
    if args.pretrain_data:
        pretrain_data = blending_datasets_denserm(
            args.pretrain_data,
    ...
    prompts_dataloader = strategy.setup_dataloader(
        prompts_dataset, args.micro_rollout_batch_size, True, True, collate_fn=lambda x: x
    )
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--init_critic_from_rm", action="store_true", default=False, help="if True, init critic from reward model"
    )
    ...
    elif "qwen3" in args.pretrain.lower() and "sft" in args.pretrain.lower():
        model_type = "qwen3"
    else:
        model_type = "debug"
    ...
    args.wandb_run_name = f"ppo_run_log_{datetime.now().strftime('%Y%m%dT%H%M')}"
