...
from openrlhf.trainer.ray.drca_actor import PolicyModelActor
from openrlhf.trainer.ray.ppo_critic import CriticModelActor
from openrlhf.utils import get_strategy
from openrlhf.utils.unit_reward_config import add_unit_reward_args, validate_unit_reward_config
...
def train(args):
    ...
    if args.async_train:
        from openrlhf.trainer.drca_trainer_async import DRCATrainerAsync as DRCATrainer
    else:
        from openrlhf.trainer.drca_trainer import DRCATrainer

    # init DRCA trainer (Single controller)
    drca_trainer = DRCATrainer.remote(
        args.pretrain,
        strategy,
        actor_model,
        critic_model,
    ...
    max_steps = ray.get(drca_trainer.get_max_steps.remote())
    ...
    ray.get(drca_trainer.fit.remote())
...
if __name__ == "__main__":
    ...
    parser.add_argument("--ckpt_path", type=str, default="./ckpt/checkpoints_drca_ray")
    ...
    parser.add_argument("--drca_beta", type=float, default=0.8, help="DRCA Reward Shaping Parameter")
    parser.add_argument(
        "--unit_extraction", type=str, default="regex", help="Unit extraction method. Can be entropy or regex."
    )
    ...
    parser.add_argument("--wandb_project", type=str, default="openrlhf_train_drca")
    parser.add_argument(
        "--wandb_run_name",
        type=str,
        default="drca_%s" % datetime.now().strftime("%m%dT%H:%M"),
    )
    ...
    # Add unit-level reward arguments
    parser = add_unit_reward_args(parser)

    args = parser.parse_args()

    # Validate unit reward configuration
    validate_unit_reward_config(args)
    ...
    if args.drca_beta:
        assert args.drca_beta > 0 and args.drca_beta <= 1, "drca_beta must be in (0, 1]"

    if args.unit_extraction:
        assert args.unit_extraction in [
            "entropy",
            "regex",
        ], "unit_extraction must be 'entropy' or 'regex'"
